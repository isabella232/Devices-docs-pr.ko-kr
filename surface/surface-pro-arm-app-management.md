...
ms.locfileid: "12449601"
---
# <a name="deploying-managing--servicing-surface-pro-x"></a>Surface Pro X 배포, 관리 및 서비스

고성능 상용 요구 사항을 처리하도록 제작된 Surface Pro X에는 동급 최강의 프로세서인 Microsoft SQ1 및 Microsoft SQ1 ARM 칩셋이 통합되어 있습니다.

3GHz CPU와 2.1 테라플롭 GPU로 구동되는 Surface Pro X는 완전한 Windows 환경을 제공합니다. 15시간의 배터리 사용 시간이 기본 제공되는 기가비트 LTE와 터치, 펜, 태블릿 및 노트북의 다양성으로 금융, 법률 및 의료 분야의 모바일 일선 작업자 및 전문가 또는 연장된 배터리 사용 시간 및 지속적인 연결 기능을 요구하는 모든 역할에 이상적입니다.
...
Autopilot 배포에는 다음과 같은 몇 가지 장점이 있습니다. 이러한 배포를 통해 공장에서 프로비전된 운영 체제를 사용하여 제로터치 배포를 간소화하고 [엔터프라이즈용 Microsoft 365 앱](https://www.microsoft.com/microsoft-365/enterprise/microsoft-365-apps-for-enterprise)의 사전 설치를 포함할 수 있습니다.

이미 현대적인 관리, 보안 및 생산성 솔루션을 사용하고 있는 조직은 Surface Pro X의 고유한 성능 기능을 활용할 수 있습니다. 현대화된 [기간 업무 앱](/microsoft-store/working-with-line-of-business-apps), [Microsoft Store(UWP)를 사용하는 고객 앱](/windows/uwp/get-started/universal-application-platform-guide) 또는 원격 데스크톱 솔루션도 이점을 얻을 수 있습니다.

## <a name="image-based-deployment-considerations"></a>이미지 기반 배포 고려 사항

MDT(Microsoft Deployment Toolkit) 및 Microsoft Endpoint Configuration Manager(이전의 System Center Configuration Manager)는 운영 체제 배포를 위해 Surface Pro X를 지원하지 않습니다. 이미지 기반 배포에 의존하는 고객은 Surface Pro 8을 고려하는 동시에 최신 배포 솔루션으로 전환할 적절한 시기를 계속 고려해야 합니다. 

## <a name="managing-surface-pro-x-devices"></a>Surface Pro X 디바이스 관리

### <a name="intune"></a>Intune

[Microsoft Intune](/intune)은 ID 및 액세스 제어를 위해 Azure Active Directory와 통합되며 등록된 Surface Pro X 장치의 세부적인 관리를 제공합니다. Intune MDM(모바일 장치 관리) 정책 설정은 Windows 그룹 정책과 같은 이전 온-프레미스 도구에 비해 몇 가지 장점이 있습니다. 여기에는 더 빠른 장치 로그인 시간과 클라우드에서 전체 장치 관리를 가능하게 하는 보다 간소화된 정책 설정 카탈로그가 포함됩니다. 예를 들어 [eSIM 프로필을 사용하여 LTE를 관리](/windows/client-management/mdm/esim-enterprise-management)하여 데이터 요금제를 구성하고 여러 장치에 활성화 코드를 배포할 수 있습니다.

### <a name="co-management"></a>공동 관리

Autopilot에 배포되면 Surface Pro X 장치를 Azure AD 또는 Active Directory([하이브리드 Azure AD 조인](/azure/active-directory/devices/concept-azure-ad-join-hybrid))에 조인할 수 있습니다. 여기서 [Intune](/mem/intune/remote-actions/device-management)으로 장치를 관리하거나 32비트 x86 ConfigMgr 클라이언트를 설치하는 [Endpoint Configuration Manager](/mem/configmgr/comanage/overview)를 사용하여 장치를 공동 관리할 수 있습니다.
...
Microsoft Defender는 지원되는 장치 수명 동안 ARM 기반 PC의 Windows 10 및 Windows 11을 보호하는 데 도움이 됩니다. 

일부 타사 바이러스 백신 소프트웨어는 ARM 기반 프로세서에서 실행되는 장치에 설치할 수 없습니다. ARM 기반 PC에서의 AV 앱 준비를 위해 타사 바이러스 백신 소프트웨어 공급자와의 협업이 계속되고 있습니다. 바이러스 백신 소프트웨어 공급자에게 문의하여 앱을 사용할 수 있는 시기를 확인하세요.
...
Surface Pro X는 Windows 업데이트를 사용하여 가정 사용자와 중소기업 사용자 모두를 위해 드라이버와 펌웨어를 최신 상태로 유지하는 작업을 단순화하도록 설계되었습니다. 기본 설정을 사용하여 자동 업데이트를 수신합니다.  확인하려면:
...
- WSUS(Windows Server Update Services)는 Surface Pro X에 대한 드라이버 및 펌웨어 제공을 지원하지 않습니다.
...
- ARM 기반 프로세서에서 실행되는 Windows 10 또는 Windows 11 PC용으로 설계된 드라이버를 사용하는 앱.
- Windows용 x64 에뮬레이션은 이제 Windows 11에서 일반 공급됩니다.
...
Azure Virtual Desktop을 사용하면 모든 위치에서 모든 컴퓨팅 디바이스 또는 플랫폼에서 Windows 데스크톱, 애플리케이션 및 데이터에 액세스할 수 있습니다. 자세한 내용은 [Azure Virtual Desktop 사이트](https://aka.ms/wvd)를 참조하세요. 
...
- Chromium 기반 Firefox 및 Microsoft Edge는 ARM 기반 Windows 10 또는 Windows 11 PC에서 향상된 성능으로 기본적으로 실행됩니다.

## <a name="installing-and-using-microsoft-office"></a>Microsoft Office 설치 및 사용

- ARM 기반 프로세서의 Windows 10 또는 Windows 11 PC에서 최상의 경험을 위해 Office 365를 사용하세요.
- Office 365 "간편 실행"은 ARM 기반 프로세서의 Windows 10 또는 Windows 11 PC에서 실행되도록 최적화된 Outlook, Word, Excel 및 PowerPoint를 설치합니다.
...
다음 표는 ARM 기반 Windows 10 또는 Windows 11이 설치된 Surface Pro X에서 선택한 주요 기능의 가용성을 보여줍니다.
...
| Surface UEFI 구성기                     | 아니오      | 펌웨어 수준에서 Surface Pro X에서 하드웨어를 비활성화할 수 있는 옵션이 없습니다.                |
...
| --------------------------------------------- | ------- | ------------------------------------------------------------------------------------- |
| BitLocker                                     | 예     |                                                                                       |
| Microsoft Defender                            | 예     |                                                                                       |
| 타사 바이러스 백신에 대한 지원             | 메모 참고| 일부 타사 바이러스 백신 소프트웨어는 ARM 기반 프로세서에 설치할 수 없습니다. 바이러스 백신 소프트웨어 공급자에게 문의하여 앱을 사용할 수 있는 시기를 확인하세요. |
| 보안 부팅                                   | 예     |                                                                                       |
