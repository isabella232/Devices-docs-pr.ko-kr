...
ms.locfileid: "12449181"
---
# <a name="manage-surface-driver-updates-in-configuration-manager"></a>Configuration Manager에서 구성 관리자 관리

## <a name="summary"></a>요약

[Microsoft System Center Configuration Manager 버전 1710](/sccm/core/plan-design/changes/whats-new-in-version-1710#software-updates)부터 Configuration Manager 클라이언트를 통해 Microsoft Surface 펌웨어 및 드라이버 업데이트를 직접 동기화하고 배포할 수 있습니다. 이 프로세스는 정기적인 업데이트 배포와 입니다. 그러나 Surface 드라이버 업데이트를 카탈로그로 다운로드하려면 몇 가지 추가 구성이 필요합니다.
...
> [!NOTE]
> 환경이 선행 요구 사항을 충족하지 않는 경우 [FAQ](#frequently-asked-questions-faq) 섹션에서 Surface 드라이버 및 펌웨어 [](https://support.microsoft.com/help/4098906/manage-surface-driver-updates-in-configuration-manager#1) 업데이트를 배포하는 다른 방법을 참조하세요.
...
Configuration Manager 로그의 전체 목록은 Log [files in System Center Configuration Manager](/sccm/core/plan-design/hierarchy/log-files).
...
1. Configuration Manager 콘솔에서 **AdministrationOverviewSite** > **** >  **ConfigurationSites로****** >  이동하십시오.
1. 해당 환경에 대한 최상위 SUP 서버가 포함된 사이트를 선택합니다.
1. 리본에서 사이트 구성 요소 구성을 **선택한** 다음 소프트웨어 업데이트 **지점을 선택합니다**. 또는 사이트를 마우스 오른쪽 단추로 클릭한 다음 **Configure Site ComponentsSoftware** >  **Update Point을 선택합니다**.
...
1. 다음 경고 메시지가 표시될 때 확인을 **선택합니다**.

   ![Configuration Manager.](images/manage-surface-driver-updates-2.png)

1. 제품 탭에서 업데이트할 제품을 선택하고 확인을 **선택합니다**.

   대부분의 드라이버는 다음 제품 그룹에 속합니다.

   - Windows 10 이상 버전 드라이버
   - Windows 10 이상 업그레이드 서비스 & 드라이버 업그레이드
   - Windows 10 업데이트 및 이후 서비스 드라이버
   - Windows 10 업데이트 및 이후 업그레이드 & 드라이버
   - Windows 10 크리에이터스 업데이트 이상 서비스 드라이버
   - Windows 10 크리에이터스 업데이트 및 이후 업그레이드 & 드라이버
   - Windows 10 Fall Creators Update 및 이후 서비스 드라이버
   - Windows 10 Fall Creators Update 및 이후 업그레이드 & 드라이버
   - Windows 10 S 이상 서비스 드라이버
   - Windows 10 S 버전 1709 이상 서비스 드라이버
   - Windows 10 S 버전 1709 이상 업그레이드 & 서비스 드라이버를 테스트합니다.

   > [!NOTE]
   > 대부분의 Surface 드라이버는 여러 Windows 10 또는 Windows 11 그룹에 속합니다. 여기에 나열된 모든 제품을 선택하지 않을 수도 있습니다. 업데이트 카탈로그를 채우는 제품 수를 줄일 수 있도록 동기화에 필요한 제품만 선택하는 것이 좋습니다.
...
1. Configuration Manager 콘솔에서 **소프트웨어 라이브러리** > **OverviewSoftware** >  **UpdatesAll 소프트웨어 업데이트** > **로 이동하세요**.
1. 리본 메뉴에서 **소프트웨어 업데이트 동기화를 선택합니다**. 또는 모든 소프트웨어 업데이트를 마우스 오른쪽 단추 **로** 클릭한 다음 소프트웨어 업데이트 동기화 **를 선택합니다**.
...
배포에 대한 자세한 내용은 System Center [2012 Configuration Manager–Part7: Software Updates (Deploy)를 참조하십시오](https://blogs.technet.microsoft.com/elie/2012/05/25/system-center-2012-configuration-managerpart7-software-updates-deploy/).
...
Configuration Manager 환경이 오프라인 상태이면 서비스 업데이트를 Configuration Manager로 가져올 때마다 새 허용 목록을 가져오게 [](/mem/configmgr/core/servers/manage/use-the-service-connection-tool) 됩니다. 또한 Configuration Manager 콘솔에 업데이트가 표시되기 전에 드라이버가 포함된 [새 WSUS](/mem/configmgr/sum/get-started/synchronize-software-updates-disconnected) 카탈로그를 가져와야 합니다. 독립 실행형 WSUS 환경에는 Configuration Manager SUP보다 많은 드라이버가 포함되어 있기 때문에 온라인 기능이 있는 Configuration Manager 환경을 설정하고 Surface 드라이버를 동기화하도록 구성하는 것이 좋습니다. 이렇게 하면 오프라인 환경과 매우 같은 작은 WSUS 내보내기 기능을 사용할 수 있습니다.
...
아니요. 업데이트를 WSUS로 가져오는 경우에도 업데이트가 허용 목록에 나열되지 않은 경우 배포를 위해 Configuration Manager 콘솔로 가져오지 않습니다. 서비스 연결 도구를 사용하여 [](/mem/configmgr/core/servers/manage/use-the-service-connection-tool) 서비스 업데이트를 Configuration Manager로 가져와 허용 목록을 업데이트해야 합니다.

**Surface 드라이버 및 펌웨어 업데이트를 배포해야 하는 대체 방법은 무엇입니까?**

대체 채널을 통해 Surface 드라이버 및 펌웨어 업데이트를 배포하는 방법에 대한 자세한 내용은 Surface 드라이버 및 펌웨어 업데이트 [관리를 참조하세요](manage-surface-driver-and-firmware-updates.md). .msi 또는 .exe 파일을 다운로드한 다음 기존 소프트웨어 배포 채널을 통해 배포하려면 Configuration Manager를 사용하여 Surface 펌웨어 업데이트 유지를 [참조하세요](/archive/blogs/thejoncallahan/keeping-surface-firmware-updated-with-configuration-manager).
