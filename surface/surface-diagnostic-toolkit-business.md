...
description: 이 항목에서는 IT 관리자가 Surface 디바이스에서 하드웨어, 소프트웨어 및 펌웨어 문제를 신속하게 조사, 문제 해결 및 해결할 수 있도록 하는 비즈니스용 Surface 진단 Toolkit 배포 및 사용하는 방법에 대해 설명합니다.
ms.prod: w10
...
Surface가 제대로 작동하지 않는 경우 비즈니스용 Microsoft Surface Toolkit(SDT)를 통해 관리자나 관리자가 문제를 찾고 해결할 수 있습니다.  비즈니스용 SDT를 사용하면 네트워크를 통해 Surface 디바이스에서 하드웨어, 소프트웨어 및 펌웨어 문제를 신속하게 조사, 문제 해결 및 해결할 수 있습니다.

> [!NOTE]
> 비즈니스용 Surface Toolkit 디바이스를 위해 제작되었습니다. 디바이스가 개인 장치로 회사 또는 학교에서 관리되지 않는 경우 [Surface Diagnostic Toolkit](https://support.microsoft.com/en-us/help/4037239/surface-fix-common-surface-problems-using-surface-diagnostic-toolkit) 실행합니다.
...
2. Surface Tools [for IT](https://www.microsoft.com/download/details.aspx?id=46703) .msi 페이지에서 SDT Windows Installer Package(.msi)를 다운로드하고 Surface 디바이스의 기본 설정 위치(예: 데스크톱)에 복사합니다.
3. 그림 1과 같이 SDT 설치 마법사가 나타납니다. **다음**을 클릭합니다.
...
- `SENDTELEMETRY` 원격 분석 데이터를 Microsoft로 전송합니다. 플래그는 사용하지 않도록 설정하거나 사용하도록 `0` 설정할 수 `1` 있습니다. 기본값은 `1` 원격 분석 전송입니다.
- `ADMINMODE` 관리 모드에서 설치할 도구를 구성합니다. 플래그는 클라이언트 모드 또는 IT 관리자 `0` `1` 모드에 허용됩니다. 기본값은 `0`입니다.
...
SDT와 SDT 앱 콘솔이 모두 에 `C:\Program Files\Microsoft\Surface\Microsoft Surface Diagnostic Toolkit for Business` 설치됩니다.
...
- [명령을 사용하여 비즈니스용 Surface Toolkit 사용](surface-diagnostic-toolkit-command-line.md)
