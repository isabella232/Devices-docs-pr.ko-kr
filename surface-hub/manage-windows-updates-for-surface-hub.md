...
New releases of the Surface Hub operating system are published through Windows Update, just like releases of Windows 10. Surface Hub에 설치되는 업데이트와 업데이트가 적용되는 시기를 관리할 수 있는 몇 가지 방법이 있습니다.
...
- **Quality updates** - Updates that focus on the installation of security fixes, drivers, and other servicing updates. Microsoft는 매월 하나의 누적 품질 업데이트를 게시할 예정입니다.
...
| --------- | --------- | --------- | --------- | --------- | --------- |
| 미리 보기(예: 중요하지 않은 또는 테스트용 장치) | Small | Windows Insider Preview | None.  | 없음.  | 새 기능을 수동으로 테스트 및 평가합니다. 문제가 있는 경우 업데이트를 일시 중지합니다. |
...
MDM을 사용하여 유지 관리 기간을 변경하려면 [SurfaceHub 구성 서비스 공급자](https://msdn.microsoft.com/library/windows/hardware/mt608323.aspx)에서 **MOMAgent** 노드를 설정합니다. See [Manage settings with an MDM provider](manage-settings-with-mdm-for-surface-hub.md) for more details.
